defmodule FlakyConnectionTest do
  ...
  test "proxying data should work" do
    ...
    conn = FlakyConnection.start(~c"localhost", 11112)
    {:ok, sock} = :gen_tcp.connect(~c"localhost", conn.port, [:binary, active: false])
  ...
  test "proxying ssl data should work" do
    ...
    conn =
      FlakyConnection.start(~c"localhost", 11113,
        ssl: [keyfile: "./test/cert/server.key", certfile: "./test/cert/server.crt"]
      )

    {:ok, sock} =
      :ssl.connect(~c"localhost", conn.port, [
        :binary,
        active: false,
        verify: :verify_peer,
        cacertfile: "./test/cert/rootCA.crt"
      ])
  ...
  test "latency" do
    ...
    conn = FlakyConnection.start(~c"localhost", 11111)
    {:ok, sock} = :gen_tcp.connect(~c"localhost", conn.port, [:binary, active: false])
    remote_sock = Task.await(task)
    :inet.setopts(remote_sock, active: :once)
    :gen_tcp.send(sock, "ping")
